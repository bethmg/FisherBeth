...
zlist=[0.0,0.5,1.0]
Rlistm=[25,30]
def PeakDict(halo_PDF_dict,PDF_forcov_dict,CosM=False):
	print('Building PDF dictionary')
	peak_z={}
	cosmo_labels=['fiducial', 'h_m', 'h_p', 'ns_m', 'ns_p', 'Ob2_m', 'Ob2_p', 'Om_m', 'Om_p', 's8_m', 's8_p']
	if CosM==True: 
		cosmo_labels=np.concatenate((cosmo_labels,['b1_m','b1_p']))
	for z in zlist:
		print('z =',z,end=': ')
		peak_cos={}
		chosen_indices={}
...
def Cov_Matrix(z,Rlist=None,PDF=None,Pk=None): 
	N_sims=1000
	data_vector=np.empty([0,N_sims])
	if Rlist!=None:
		for R in Rlist:
			data_vector=np.concatenate((data_vector,PDF[z]['cov'][R]))
	if Pk!=None:
		data_vector=np.concatenate((data_vector,Pk[z]['cov']))
	cov_matrix=np.cov(data_vector)
	return(cov_matrix)

def Corr_Matrix(z,Rlist=None,PDF=None,Pk=None):
	N_sims=1000
	data_vector=np.empty([0,N_sims])
	if Rlist!=None:
		for R in Rlist:
...
def Deriv(param,z,R=None,PDF=None,Pk=None):
	...
	if param=='b1': # not sure why need this bit? 
		vec_p=PDF[z][param+'_p'][R][:100]
		vec_m=PDF[z][param+'_m'][R][:100]
		vec_deriv=(vec_p-vec_m)/(2*dlt_theta[param])
	else:
		if R!=None:
			vec_p=PDF[z][param+'_p'][R]
			vec_m=PDF[z][param+'_m'][R]
		elif Pk!=None:
			vec_p=Pk[z][param+'_p']
			vec_m=Pk[z][param+'_m']   
		vec_deriv=(vec_p-vec_m)/(2*dlt_theta[param])
...
#PDF_dict={}
#Pk_dict={}
def Deriv_Vector(z,param,Rlist=None,PDF=None,Pk=None):
	vec=np.array([])
	# for param in param_labels:
	if Rlist!=None:
		for R in Rlist:
			vec=np.concatenate((vec,Deriv(param,z,R,PDF,Pk))) 
	if Pk!=None:
		vec=np.concatenate((vec,Deriv(param,z,PDF=PDF,Pk=Pk)))
	return vec
...
#PDF_dict={}
#Pk_dict={} 
def Fisher(zlist,param_labels,Rlist=None,PDF=None,Pk=None):
	N_sims=1000
	fisher=np.zeros([len(param_labels),len(param_labels)])
	for z in zlist:
		cov_matrix=Cov_Matrix(z,Rlist,PDF,Pk)
		hartlap=float(N_sims - 2 - len(cov_matrix[:,0]))/float(N_sims - 1)
		derivs_vector={}
		for param in param_labels:
			derivs_vector[param]=Deriv_Vector(z,param,Rlist,PDF,Pk)
...
def Param_Covariance(zlist,param_labels,Rlist=None,PDF=None,Pk=None):
	mat=np.linalg.inv(Fisher(zlist,param_labels,Rlist,PDF,Pk))
	return mat
